...
import { 
  ...
  Trash2, Download, Info, AlertTriangle, FileUp, Users 
} from 'lucide-react';
...
export default function ParentLeavePage() {
  ...
  const fetchChildren = async () => {
    try {
      ...
      if (data && data.length > 0) {
        ...
        setForm(prev => ({ ...prev, student_id: data[0].id }));
      }
    } catch (err) {
  ...
  const loadLeaves = async () => {
    ...
    try {
      ...
      toast.show('error', 'Error', err.message || 'Failed to load leave history.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleApplyLeaveSubmit = async (e) => {
    ...
    // Validations
    const errors = {};
    if (isParent && !form.student_id) errors.student_id = 'Please select a child.';
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-black text-text-primary tracking-tight font-display">Student Leave Requests</h2>
          <p className="text-xs text-text-muted">Apply for leave and monitor approval history for your child.</p>
        </div>
        <Button 
      ...
      {/* Children Filter (only for parents with multiple children) */}
      {isParent && children.length > 1 && (
        <div className="flex items-center gap-3 bg-surface border border-border p-4 rounded-xl">
          <Users className="h-4 w-4 text-text-muted" />
          <span className="text-xs font-bold text-text-secondary">View Child:</span>
          <Select
          ...
          >
            ...
            {children.map(c => (
              <option key={c.id} value={c.id}>{c.name} ({c.class_name || 'N/A'})</option>
            ))}
          </Select>
        </div>
      )}
        ...
        <div className="overflow-x-auto min-h-[200px]">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-[10px] text-text-muted mt-0.5">Click the "Apply Student Leave" button to submit your first leave application.</p>
            </div>
          ) : (
              ...
              <tbody className="divide-y divide-border">
                {leaves.map(lr => (
                  <tr key={lr.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-900/10">
                    <td className="p-3">
                      <span className="font-bold text-text-primary">{lr.student_name}</span>
                    </td>
                    <td className="p-3">
                      <span className="font-semibold text-text-secondary">{lr.leave_type}</span>
                    </td>
                    <td className="p-3">
          ...
          <form onSubmit={handleApplyLeaveSubmit} className="space-y-4">
            {isParent && children.length > 1 && (
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-text-secondary uppercase">Select Child</label>
                <Select
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                placeholder="Type the reason for leaf request here (max 100 words)..."
                rows={4}
                value={form.reason}
          ...
          <div className="space-y-4">
            <div className="flex gap-3 text-text-secondary items-start">
              ...
              <p className="text-xs leading-relaxed">
                Are you sure you want to cancel the leave request for <span className="font-bold">{selectedLeave.student_name}</span> from <span className="font-bold">{selectedLeave.from_date}</span> to <span className="font-bold">{selectedLeave.to_date}</span>? This action cannot be undone.
              </p>
            </div>

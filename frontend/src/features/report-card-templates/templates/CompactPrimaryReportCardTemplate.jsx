...
export default function CompactPrimaryReportCardTemplate({ data, config = {} }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between border-b-2 border-amber-400 pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {school.address && (
              <p className="text-[10.5px] font-semibold text-amber-800 uppercase tracking-wide">
                {school.address}
              </p>
            )}
      ...
      {/* Marks Table Container - Expands dynamically for small subject counts */}
      <div className="border border-amber-200 rounded-xl overflow-hidden shadow-2xs flex-1 flex flex-col justify-between">
        {isFinalReport ? (
          <table className="w-full h-full text-left border-collapse" style={{ fontSize: fontSizePx }}>
            ...
            <tbody className="divide-y divide-amber-100 font-medium">
              {subjects.map((sub, idx) => (
                <tr key={idx} className={idx % 2 === 0 ? 'bg-white' : 'bg-amber-50/30'}>
                  ...
                  {(data.session_exams || ['Quarterly Exam', 'Half Yearly Exam', 'Annual Exam']).map(exName => {
                    ...
                    return (
                      <React.Fragment key={exName}>
                        <td style={{ padding: cellPadding }} className="text-center border-r border-amber-100 font-mono text-zinc-600">
                          {score ? score.max_marks : '—'}
                        </td>
                        <td style={{ padding: cellPadding }} className="text-center border-r border-amber-100 font-mono font-bold text-amber-800">
              ...
              <tr className="bg-amber-100 border-t-2 border-amber-500 font-bold text-amber-950">
                ...
                {(data.session_exams || ['Quarterly Exam', 'Half Yearly Exam', 'Annual Exam']).map(exName => (
                  <React.Fragment key={exName}>
                    <td style={{ padding: cellPadding }} className="text-center border-r border-amber-200 font-mono">{data.exam_totals?.[exName]?.max_marks ?? 0}</td>
                    <td style={{ padding: cellPadding }} className="text-center border-r border-amber-200 font-mono font-bold text-amber-950">{data.exam_totals?.[exName]?.marks_obtained ?? 0}</td>
                  </React.Fragment>
                ))}
      ...
      {(() => {
        ...
        return (
          <div className="mt-2.5 font-sans text-xs leading-normal border border-amber-100 rounded-lg p-3 bg-amber-50/30">
      ...
      <div className="pb-1 font-sans flex justify-between items-end text-xs font-bold text-zinc-700 px-6" style={{ marginTop: '75px' }}>

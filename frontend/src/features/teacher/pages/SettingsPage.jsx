...
import { 
  ...
  Settings, ChevronRight, School, Laptop, Check
} from 'lucide-react';
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    // Fetch user details from /api/auth/profile
  ...
  const departmentDisplay = profileData?.department ? `${profileData.department} Teacher` : 'Teacher';
    ...
    <div className="space-y-6 max-w-2xl mx-auto pb-16 animate-in fade-in duration-300">
      ...
      <Card className="bg-surface border-border overflow-hidden shadow-xs">
        <CardContent className="p-6 flex flex-col items-center text-center space-y-4">
          ...
          <div>
            <h3 className="text-base font-bold text-text-primary">{user?.name || 'Teacher Portal'}</h3>
            <p className="text-xs text-text-muted font-semibold mt-0.5">{departmentDisplay}</p>
          </div>
        </CardContent>
      </Card>
        ...
        >
          <div className="space-y-4 text-xs">
            <div className="grid grid-cols-3 border-b border-border/50 py-2">
              ...
              <span className="col-span-2 text-text-primary font-bold">{user?.name}</span>
            </div>
            <div className="grid grid-cols-3 border-b border-border/50 py-2">
            ...
            <div className="grid grid-cols-3 border-b border-border/50 py-2">
              <span className="font-bold text-text-muted uppercase">Email ID</span>
              <span className="col-span-2 text-text-primary font-semibold">{profileData?.email || 'Not Provided'}</span>
            </div>
            <div className="grid grid-cols-3 border-b border-border/50 py-2">
              <span className="font-bold text-text-muted uppercase">Staff ID</span>
              <span className="col-span-2 text-text-primary font-semibold">{profileData?.employee_id || 'Not Assigned'}</span>
            </div>
            <div className="grid grid-cols-3 py-2">
              <span className="font-bold text-text-muted uppercase">Role</span>
              <span className="col-span-2 text-text-primary font-bold">{user?.role}</span>
            </div>
        ...
        >
          <div className="space-y-3">
            <p className="text-xs text-text-secondary leading-relaxed">
              Your account details are fully secured under end-to-end database encryption. To request a password reset or modification of profile records, please reach out to your School Administrator.
            </p>
            <div className="flex justify-end pt-3 border-t border-border mt-4">
              <Button onClick={() => setActiveModal(null)} variant="outline">Close</Button>
            </div>
